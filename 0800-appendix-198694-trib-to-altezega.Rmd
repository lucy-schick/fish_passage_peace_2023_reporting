...
upstream from the confluence with Altezega Creek.
...
\@ref(tab:tab-culvert-bcfp-198694). A map of the watershed is provided in
map attachment
...
[@moe2011] (Table \@ref(tab:tab-culvert-198694)). There
...
`r if(gitbook_on){knitr::asis_output("(Figures \\@ref(fig:photo-198694-03) - \\@ref(fig:photo-198694-06)).")}else(knitr::asis_output("(Figure \\@ref(fig:photo-198694-d02))."))`
...
```{r photo-198694-05-prep}
my_photo1 = fpr::fpr_photo_pull_by_str(str_to_pull = '_u3_')

my_caption1 = paste0('Typical habitat upstream of PSCIS crossing ', my_site, '.')
```

```{r photo-198694-05, fig.cap= my_caption1, out.width = photo_width, eval=gitbook_on}
grid::grid.raster(jpeg::readJPEG(my_photo1))
```

<br>

```{r photo-198694-06-prep}
...
```{r photo-198694-06, fig.cap= my_caption2, out.width = photo_width, eval=gitbook_on}
grid::grid.raster(jpeg::readJPEG(my_photo2))
```

```{r photo-198694-d04, fig.cap = my_caption, fig.show="hold", out.width= c("49.5%","1%","49.5%"), eval=identical(gitbook_on, FALSE)}
